...
export const convertCollectiblesResponseToCollectiblesData = (collectiblesResponse: ICollectiableResponse[]): ICollectibleData[] => {
    return collectiblesResponse.filter(collectible => collectible.categories && collectible.categories.includes(eCollectibleCategories.Collectibles)).map(collectible => {
        return {
            id: collectible.id,
